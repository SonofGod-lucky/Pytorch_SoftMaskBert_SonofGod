...
class SMBertMlm(nn.Module):
    ...
    def load_pretrain(self, path=PretrainPath):
        ...
        finetuen_model_dict = self.state_dict()
        new_parameter_dict = {}
        ## 加载embedding层参数
        for key,target in local2target_emb.items():
            new_parameter_dict[key] = pretrain_model_dict[target]
        ## 加载transfomerblock层参数
        for i in range(self.num_hidden_layers):
            for key,target in local2target_transformer.items():
                key = key % i
                target = target % i
                new_parameter_dict[key] = pretrain_model_dict[target]
        finetuen_model_dict.update(new_parameter_dict)
        self.load_state_dict(finetuen_model_dict)
